...
| `1027` | `insufficient_balance` | 422 | Debit exceeds available (or, on payout create, the reserve cannot cover the amount) |
...
| `1043` | `idempotency_conflict` | 409 | `Idempotency-Key` reused with a different raw body |
| `1044` | `webhook_timestamp_expired` | 401 | Webhook `t` outside `WEBHOOK_TOLERANCE_SECONDS` |
...
- **`1027`**: Raised on `POST /v1/payouts` when `available` cannot cover the payout (no row is inserted). The payout job may still record `1027` if `pending` cannot cover confirm — that path is exceptional after a successful reserve.
- **`1043`**: The original create is not replayed. Clients must use a new key (or the exact same raw body) to proceed.
- **`1044`**: The HMAC `v1` matched but `|now - t|` exceeded the tolerance window (default 300 seconds). Missing or invalid signatures stay generic HTTP 401 without this code.
